...
func (s *resourceGroupProxyServer) AddResourceGroup(ctx context.Context, req *resource_manager.PutResourceGroupRequest) (*resource_manager.PutResourceGroupResponse, error) {
	...
	if s.manager == nil {
		return nil, status.Error(codes.Internal, "resource manager metadata manager is not initialized")
	}
	if err := s.manager.AddResourceGroup(req.GetGroup()); err != nil {
		return nil, err
	}
	return &resource_manager.PutResourceGroupResponse{
		Body: "Success!",
	}, nil
}
...
func (s *resourceGroupProxyServer) ModifyResourceGroup(ctx context.Context, req *resource_manager.PutResourceGroupRequest) (*resource_manager.PutResourceGroupResponse, error) {
	...
	if err := s.manager.ModifyResourceGroup(req.GetGroup()); err != nil {
		return nil, err
	}
	return &resource_manager.PutResourceGroupResponse{
		Body: "Success!",
...
func (s *resourceGroupProxyServer) DeleteResourceGroup(ctx context.Context, req *resource_manager.DeleteResourceGroupRequest) (*resource_manager.DeleteResourceGroupResponse, error) {
	...
	if err := s.manager.DeleteResourceGroup(rm_server.ExtractKeyspaceID(req.GetKeyspaceId()), req.GetResourceGroupName()); err != nil {
		return nil, err
	}
	return &resource_manager.DeleteResourceGroupResponse{
		Body: "Success!",
	}, nil
}
